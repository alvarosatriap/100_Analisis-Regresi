...
data <- read_excel("C:\\Users\\HP\\Downloads\\NSC-2025 (1).xlsx", sheet="Sheet2")
...
```{r}
plot(data$X8, data$Y,
     main = "Tingkat Partisipasi Angkatan Kerja vs Indeks Pembangunan Manusia",
     xlab = "TPAK",
     ylab = "IPM")
```
...
$$\hat{Y} = 109.1187967  -0.466211899 X_8$$

dengan Y adalah Indeks Pembangunan Manusia (IPM) dan X8 adalah Tingkat Partisipasi Angkatan Kerja (TPAK).

Didapatkan nilai $\hat\beta_0 = 109.1187967$ artinya nilai dugaan rataan Indeks Pembangunan Manusia ketika Tingkat Partisipasi Angkatan Kerja (TPAK) bernilai 0 persen adalah sebesar 109.12. Namun, karena nilai TPAK = 0 persen tidak berada dalam selang pengamatan data, maka nilai $\hat\beta_0$ tidak dapat diinterpretasikan secara praktis.

Nilai $\hat\beta_0 = -0.466211899$ artinya setiap kenaikan Tingkat Partisipasi Angkatan Kerja (TPAK) sebesar 1 persen akan diikuti oleh penurunan nilai dugaan Indeks Pembangunan Manusia sebesar 0.47 poin, dengan asumsi faktor lain dianggap konstan.

Koefisien determinasi yang dihasilkan adalah 0.1887346 yang berarti bahwa sebesar 18,873% keragaman Indeks Pembangunan Manusia dapat dijelaskan oleh Tingkat Partisipasi Angkatan Kerja (TPAK), sedangkan sisanya dijelaskan oleh faktor lain di luar model.
